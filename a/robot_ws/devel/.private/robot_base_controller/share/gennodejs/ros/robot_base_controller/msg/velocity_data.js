// Auto-generated. Do not edit!

// (in-package robot_base_controller.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class velocity_data {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.front_left_wheel = null;
      this.front_right_wheel = null;
      this.rear_left_wheel = null;
      this.rear_right_wheel = null;
    }
    else {
      if (initObj.hasOwnProperty('front_left_wheel')) {
        this.front_left_wheel = initObj.front_left_wheel
      }
      else {
        this.front_left_wheel = 0.0;
      }
      if (initObj.hasOwnProperty('front_right_wheel')) {
        this.front_right_wheel = initObj.front_right_wheel
      }
      else {
        this.front_right_wheel = 0.0;
      }
      if (initObj.hasOwnProperty('rear_left_wheel')) {
        this.rear_left_wheel = initObj.rear_left_wheel
      }
      else {
        this.rear_left_wheel = 0.0;
      }
      if (initObj.hasOwnProperty('rear_right_wheel')) {
        this.rear_right_wheel = initObj.rear_right_wheel
      }
      else {
        this.rear_right_wheel = 0.0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type velocity_data
    // Serialize message field [front_left_wheel]
    bufferOffset = _serializer.float64(obj.front_left_wheel, buffer, bufferOffset);
    // Serialize message field [front_right_wheel]
    bufferOffset = _serializer.float64(obj.front_right_wheel, buffer, bufferOffset);
    // Serialize message field [rear_left_wheel]
    bufferOffset = _serializer.float64(obj.rear_left_wheel, buffer, bufferOffset);
    // Serialize message field [rear_right_wheel]
    bufferOffset = _serializer.float64(obj.rear_right_wheel, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type velocity_data
    let len;
    let data = new velocity_data(null);
    // Deserialize message field [front_left_wheel]
    data.front_left_wheel = _deserializer.float64(buffer, bufferOffset);
    // Deserialize message field [front_right_wheel]
    data.front_right_wheel = _deserializer.float64(buffer, bufferOffset);
    // Deserialize message field [rear_left_wheel]
    data.rear_left_wheel = _deserializer.float64(buffer, bufferOffset);
    // Deserialize message field [rear_right_wheel]
    data.rear_right_wheel = _deserializer.float64(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 32;
  }

  static datatype() {
    // Returns string type for a message object
    return 'robot_base_controller/velocity_data';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '6398bc5e122606acb80413c64aee32c3';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    # velocity_data.msg
    float64 front_left_wheel
    float64 front_right_wheel
    float64 rear_left_wheel
    float64 rear_right_wheel
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new velocity_data(null);
    if (msg.front_left_wheel !== undefined) {
      resolved.front_left_wheel = msg.front_left_wheel;
    }
    else {
      resolved.front_left_wheel = 0.0
    }

    if (msg.front_right_wheel !== undefined) {
      resolved.front_right_wheel = msg.front_right_wheel;
    }
    else {
      resolved.front_right_wheel = 0.0
    }

    if (msg.rear_left_wheel !== undefined) {
      resolved.rear_left_wheel = msg.rear_left_wheel;
    }
    else {
      resolved.rear_left_wheel = 0.0
    }

    if (msg.rear_right_wheel !== undefined) {
      resolved.rear_right_wheel = msg.rear_right_wheel;
    }
    else {
      resolved.rear_right_wheel = 0.0
    }

    return resolved;
    }
};

module.exports = velocity_data;
