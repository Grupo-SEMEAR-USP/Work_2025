...
class ZigZagSearch:
    ...
    def perform_zigzag(self):
        ...
        while self.movement_active and (rospy.Time.now() - self.last_time).to_sec() < self.timeout:
            ...
            # # rospy.loginfo("Movendo para a frente por 1.1 segundos.")
            # self.pub_move_time.publish("direita_90,0.4")
            # self.pub_move_time.publish("parar,0.1")
            # rospy.sleep(2)
            ...
            rospy.loginfo("Executando movimento de procura...")
            # rospy.loginfo(f"procurando {self.aruco_reference_id}")
                
            if not self.movement_active:
                self.fb = True
                return

            # rospy.loginfo("Movendo para o lado por 0.7 segundos.")
            self.pub_move_time.publish("frente,0.7")
            self.pub_move_time.publish("parar,3.0")
            rospy.sleep(2)
...
#             # rospy.loginfo("Movendo para o lado por 0.7 segundos.")
#             self.pub_move_time.publish(self.direction+",1.0")
#             self.pub_move_time.publish("parar,3.0")
#             rospy.sleep(2)
